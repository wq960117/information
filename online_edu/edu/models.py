...
class User_course(models.Model):  # 学习记录表
    ...
    section_id = models.Foriengkey(Section, null=True, on_delete=models.SET_NULL)  # 章节外键
...
class Admin(models.Model):  # 管理员用户表
    username = models.CharField(max_length=50, unique=True)  # 管理员用户名
    password = models.CharField(max_length=50)  # 管理员密码
    roles_id = models.ForeignKey(Roles, on_delete=models.SET_NULL, null=True)  # 外键关联角色表
...
class Act(models.Model):
    ...
    data = models.DateTimeField  # 活动日期
...
class Sk(models.Model):
    ...
    price = models.DecimalField(7, 2)  # 秒杀价格
...
class Log(models.Model):
    dates = models.DateTimeField  # 执行时间
...
class Forum(models.Model):
    ...
    condition = models.DecimalField(7, 2)  # 满多少钱可以使用
    money = models.DecimalField(7, 2)  # 优惠券金额
